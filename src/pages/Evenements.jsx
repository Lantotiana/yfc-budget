...
export default function Evenements({ user, userData }) {
  ...
  // "Past" = end of event (dateFin + heureFin) is before now
  // Countdown counts down to start (dateDebut + heureDebut)
  const upcoming = []
  const past = []
          ...
          return (
            <div
              ...
              style={{
                ...
                border: isPast ? '1.5px solid transparent' : '1.5px solid rgba(94,234,212,0.35)',
                opacity: isPast ? 0.7 : 1,
              }}
            >
              {!isPast && (
                <div style={{position:'absolute', left:0, top:0, bottom:0, width:'4px', background:'#5eead4', borderRadius:'4px 0 0 4px'}} />
              )}

              <div style={{display:'flex', alignItems:'flex-start', gap:'12px', paddingLeft: isPast ? 0 : '8px'}}>
                <div style={{flex:1, minWidth:0}}>
                  <div style={{display:'flex', alignItems:'center', gap:'8px', marginBottom:'6px', flexWrap:'wrap'}}>
                    ...
                    {isPast ? (
                      <span style={{fontSize:'10px', fontWeight:'700', padding:'2px 8px', borderRadius:'20px', background:'rgba(255,255,255,0.07)', color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'.06em'}}>
                        Terminé
                      </span>
                    ) : countdown && (
                      <span style={{fontSize:'10px', fontWeight:'700', padding:'2px 8px', borderRadius:'20px', background:'rgba(94,234,212,0.15)', color:'#5eead4', textTransform:'uppercase', letterSpacing:'.06em', whiteSpace:'nowrap'}}>
                        Événement {countdown}
                      </span>
                    )}
                  </div>
                ...
                <button
                  ...
                  style={{background:'var(--del-btn-bg)', border:'none', borderRadius:'8px', padding:'6px 8px', cursor:'pointer', color:'#be123c', display:'flex', alignItems:'center', flexShrink:0}}
                >
                  <Trash2 size={15} />
                </button>
              </div>
        ...
        <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', zIndex:100, display:'flex', alignItems:'flex-end'}} onClick={closeSheet}>
          <div style={{width:'100%', background:'var(--card-bg)', borderRadius:'20px 20px 0 0', padding:'1.5rem', paddingBottom:'max(1.5rem, env(safe-area-inset-bottom))', maxHeight:'90vh', overflowY:'auto'}} onClick={ev => ev.stopPropagation()}>
            ...
            <div style={{display:'flex', gap:'10px', marginTop:'1.5rem'}}>
              ...
              <button onClick={save} disabled={saving || !form.nom.trim() || !form.dateDebut} style={{flex:2, padding:'13px', border:'none', borderRadius:'12px', background:'var(--btn-primary-bg)', color:'#fff', fontWeight:'700', fontSize:'14px', cursor:'pointer', fontFamily:'inherit', opacity:(saving || !form.nom.trim() || !form.dateDebut) ? 0.6 : 1}}>
        ...
        <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:200, display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem'}}>
          <div style={{background:'var(--card-bg)', borderRadius:'20px', padding:'1.5rem', width:'100%', maxWidth:'320px'}}>
            ...
            <div style={{display:'flex', gap:'10px'}}>
              ...
              <button onClick={confirmDelete} style={{flex:1, padding:'12px', border:'none', borderRadius:'12px', background:'#be123c', color:'#fff', fontWeight:'700', cursor:'pointer', fontFamily:'inherit'}}>
                Supprimer
              </button>
            </div>
          </div>
